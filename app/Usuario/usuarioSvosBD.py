...
from app.Usuario.usuarioModelo import Usuario

#   SERVICIOS. Del componente seguridadSvos.py importa la clase Seg_EncriptacionSvos
from app.Seguridad.seguridadSvos import Seg_EncriptacionSvos, Seg_TokenSvos

#   UTILERIAS. Del componente baseDatos.py importa la funcion conexionBD
from app.Utilerias.BaseDatosModelo import PostgresBase
...
class UsuarioSvos():
    @classmethod
    def alta_usuario(cls, user, arg_current_app):
        try:
            # Conexion a la BBDD (Postgres)    
            connection = PostgresBase(arg_current_app)
    ...
    @classmethod
    def valida_usuario(cls, _version, user, arg_current_app, validaPass=True, vigencia=60):    # Vigencia del Token (vigencia) es opcional, su default es 60 minutos
        try:
            # Conexion a la BBDD (Postgres)    
            connection = PostgresBase(arg_current_app)
            #cursor.execute(''' select * from usuario where email = %s ''', [user.email]) 

            connection.execute(''' SELECT u.id, u.email, u.fullname, u."password", u.vigente,  
    ...
    @classmethod
    def lista_usuarios(cls, arg_current_app):
        try:
            datosRespuesta = {
                'error': False, 
                'mensaje': ''
    ...
    @classmethod
    def consulta_usuario(cls, user, arg_current_app): 
        try:
            # Conexion a la BBDD (Postgres)    
            connection = PostgresBase(arg_current_app)

            connection.execute(''' select * from usuario where email = %s ''', [user.email]) 
            regto = connection.fetchall()
            connection.close()
    ...
    @classmethod
    def roles_usuario(cls, arg_current_app, user):
        try:
            # Conexion a la BBDD (Postgres)    
            connection = PostgresBase(arg_current_app)
